...
func (c *covert) buildBak() error {
	...
	// 解压
	//name := path.Base(c.input)
	err = utils.UnZip(c.input, fmt.Sprintf("./%s", c.contract))
	if err != nil {
		return err
	}
	c.logger.Info("unzip success", zap.String("name", c.contract))
	in, err := os.Open(fmt.Sprintf("./%s/%s", c.contract, "index.html"))
	if err != nil {
		return err
	}
	defer in.Close()

	out, err := os.OpenFile(fmt.Sprintf("./%s/%s", c.contract, "index.html.bak"), os.O_RDWR|os.O_CREATE, 0766)
	if err != nil {
		return err
	}
	defer out.Close()
	...
	for {
		...
		if strings.Contains(newLine, "周梓良") {
			newLine = strings.Replace(newLine, "周梓良", "贺兴", -1)
		}
		if strings.Contains(newLine, "武汉增强子生物科技有限公司") {
			newLine = strings.Replace(newLine, "武汉增强子生物科技有限公司", c.address, -1)
		}
		if strings.Contains(newLine, "派森诺生物科技有限公司") {
			newLine = strings.Replace(newLine, "派森诺生物科技有限公司", "武汉增强子生物科技有限公司", -1)
		}
		if strings.Contains(newLine, "派森诺基因预付款技术服务合同HX") {
			newLine = strings.Replace(newLine, "派森诺基因预付款技术服务合同HX", c.contract, -1)
		}
		if strings.Contains(newLine, "YFnj") {
			temp := strings.Index(newLine, "YFnj")
			c.logger.Info("index ", zap.String("temp", newLine[temp:temp+12]))
			newLine = strings.Replace(newLine, newLine[temp:temp+12], c.order, -1)
		}
