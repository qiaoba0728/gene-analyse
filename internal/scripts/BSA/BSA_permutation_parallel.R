...
write.table(format_dat,file="/data/output/format_final_dat.txt",sep="\t",quote=F,row.names=F,col.names=F)

pdf("/data/output/Average_Delta_with_confidence_interval.pdf",width=14,height=3.5)
...
points(format_dat[,c(1,2)],pch=20,cex=0.8)
...
## delata 
pdf("/data/output/Average_Delta_with_confidence_interval_adjuest.pdf",width=14,height=3.5)
...
dev.off()
## w 
pdf("/data/output/Average_Delta_with_confidence_interval_w.pdf",width=14,height=3.5)
...
points(format_dat[,c(1,5)],pch=20,cex=0.8)
axis(side=1,labels=paste("Chr",1:(length(chromosome_len)-1),sep=""),at=text_pos,tick=0,cex.axis=c_n)
for (i in 2:(length(chromosome_len)-1)) {
	tmp <- sum(chromosome_len[1:i])
	abline(v=tmp,col="gray",lty=2,lwd=2)
...
dev.off()
## p
pdf("/data/output/Average_Delta_with_confidence_interval_p.pdf",width=14,height=3.5)
...
points(format_dat[,c(1,6)],pch=20,cex=0.8)
axis(side=1,labels=paste("Chr",1:(length(chromosome_len)-1),sep=""),at=text_pos,tick=0,cex.axis=c_n)
for (i in 2:(length(chromosome_len)-1)) {
	tmp <- sum(chromosome_len[1:i])
	abline(v=tmp,col="gray",lty=2,lwd=2)
...
pdf("/data/output/Average_Delta_with_confidence_interval_0.05.pdf",width=14,height=3.5)
...
pdf("/data/output/Average_Delta_with_confidence_interval_base0.1.pdf",width=14,height=3.5)
...
pdf("/data/output/Average_Delta_with_confidence_interval_base0.1_0.05.pdf",width=14,height=3.5)
plot(format_dat[,c(1,2)],pch=20,bty="l",ylab="Average delta value",xlab="Chromosome",xaxt="n",xaxs="i",yaxs="i",ylim=c(0.1,max(format_dat[,2],na.rm=T)+0.05))
abline(h=0.1)
axis(side=1,labels=paste("Chr",1:(length(chromosome_len)-1),sep=""),at=text_pos,tick=0,cex.axis=c_n)
for (i in 2:(length(chromosome_len)-1)) {
